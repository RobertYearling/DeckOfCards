...
namespace DeckOfCards.Models
{
    public class Deck
    {
        public List<Card> Look = new List<Card>(); // Object Construction
        ...
        public Card Deal()  // Method - removes from list and returns card
        {
            Card SelectedCard = Look[0];
            Look.RemoveAt(0);
            return SelectedCard;
        }

        public void Shuffle()  // Randomly Reorders the deck of cards
        {
            Look = Look.OrderBy(x => Guid.NewGuid()).ToList();
        }

        public void Reset()  // Resets the Cards to the original 52
        {
            Look.Clear();
            Look = new List<Card>()
            {
